...
const mainMatch = /<main className="flex-1 flex overflow-hidden relative">[\s\S]*?<\/main>/;

const newMain = `<main className="flex-1 flex overflow-hidden relative">
  {viewMode === "umap" && (
...
code = code.replace(mainMatch, newMain);
